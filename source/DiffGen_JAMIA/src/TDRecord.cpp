...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
int CTDRecord::hasConceptInRange(CTDConcept* pConcept)
{
	int minimum = pConcept->m_minimum;
	int maximum = pConcept->m_maximum;

	CTDConcepts* pRawConcepts = getValue(pConcept->getAttrib()->m_attribIdx)->getRawConcepts();
	int size = pRawConcepts->GetSize();

	CTDConcept* pSetValue = NULL;

	for(int i=0; i< size; i++){
		pSetValue = pRawConcepts->GetAt(i);
		if(pSetValue->m_maximum <= maximum && pSetValue->m_minimum >= minimum)
			return 1;
	}
