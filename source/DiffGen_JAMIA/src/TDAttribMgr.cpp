    ...
    try {
        ...
        while (attribFile.ReadString(lineStr)) {
			...
			if (attribName.CompareNoCase(TD_VID_ATTRIB_NAME) != 0) {
                // Create an attribute.
				ASSERT(attribType == TD_CONTINUOUS_ATTRIB || attribType == TD_DISCRETE_ATTRIB || attribType == TD_SETVALUED_ATTRIB);
                ...
                if (attribType.CompareNoCase(TD_CONTINUOUS_ATTRIB) == 0)
                    ...
                    pNewAttribute = new CTDSetAttrib(attribName);
                ...
                if (attribName.CompareNoCase(TD_CLASSES_ATTRIB_NAME) != 0) {
					...
					pNewAttribute->m_bVirtualAttrib = true;    //  Previously, m_bVirtualAttrib is set to "true" inside the "CTDVirtualID::initVID" function 
                }
                else {
    ...
    try {
        ...
        for (int a = 0; a < nAttributes - 1; ++a) {
			...
			if (pAttrib->isSetValued()){
				CTDHierarchyCuts* pFlattenHierarchyCuts = pAttrib->getHierarchyCuts();
				int nHierarchyCuts = pFlattenHierarchyCuts->GetSize();

				for (c = 0; c < nHierarchyCuts; ++c) {
					nameFile.WriteString(pFlattenHierarchyCuts->GetAt(c)->m_conceptsValues.toString());
					if (c < nHierarchyCuts - 1)
						nameFile.WriteString(CString(TD_NAMEFILE_SEPARATOR) + _T(" "));
					else {
						if (pFlattenHierarchyCuts->GetSize() == 1) {
...
{    
    ...
    for (int a = 0; a < nAttribs; ++a) {
        ...
        for (POSITION conPos = pAttrib->m_cut.GetHeadPosition(); conPos != NULL;) {
			...
			// We already so this checking for all attributes excepts set-valued during the Support Matrix construction. 
			// For Set-valued attribute, we add this here again. 
			if (pCutConcept->getNumChildConcepts() == 0) {
				pCutConcept->m_bCutCandidate = false;  // "true" by default.
				continue;
			}

            if (!pCutConcept->m_bCutCandidate)
                continue;
            if (!pCutConcept->computeScore(getNumClasses(), nAttribs))
                return false;
        }
    }
    return true;
