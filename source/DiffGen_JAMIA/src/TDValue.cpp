...
CTDValue::CTDValue()
    : m_pCurrConcept(NULL), m_pCurrHierarchyCut(NULL)
{
...
bool CTDValue::initConceptToRoot(CTDAttrib* pAttrib)
{
	...
	if (pAttrib->isSetValued())	
		m_pCurrHierarchyCut = pAttrib->getHierarchyCutRoot();
	else 
...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
bool CTDValue::setSplitCurHierarchyCut(CTDHierarchyCut* pSplitHierarchyCut, int childInd)
{
    ASSERT(pSplitHierarchyCut);
	 m_pCurrHierarchyCut = pSplitHierarchyCut->getChildHierarchyCut(childInd); 
	return true;
}
...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
bool CTDValue::setCurHierarchyCut(CTDHierarchyCut* pCurHierarchyCut)
{
    ASSERT(pCurHierarchyCut);
	m_pCurrHierarchyCut = pCurHierarchyCut; 
    return true;
}
...
CTDSetValue::CTDSetValue()
{
	m_pRawConcepts.cleanup();
}

//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
CString CTDSetValue::toString(bool bRawValue)
{
	if (bRawValue)
        return getRawConcepts()->toString();
	else
		return m_pCurrHierarchyCut->toString();
}

bool CTDSetValue::buildBitValue(const CString& rawVal, CTDAttrib* pAttrib)
{
    
	m_pRawConcepts.cleanup();

	if (rawVal.IsEmpty()){
		cerr << _T("CTDDataMgr: Empty set value string in record: ") << rawVal << endl;
		ASSERT(false);
        return false;
	}
	...
	CString valueStr;
	int FLAG = 0;
	CBFStrParser strParser(rawVal, TD_SETVALUE_DELIMETER);
	while (strParser.getNext(valueStr)) {
		// Check unknown value
		valueStr = valueStr.Mid(1, valueStr.GetLength() - 1);
		...
		int nFlattenConcepts =  pAttrib->getFlattenConcepts()->GetSize();
		FLAG = 0;
		for (int i = nFlattenConcepts - 1; i >= 0; --i) {
			pConcept = pAttrib->getFlattenConcepts()->GetAt(i);
			if (!pConcept) {
				ASSERT(false);
				return false;
			}
			if (valueStr.CompareNoCase(pConcept->m_conceptValue) == 0){ 
				m_pRawConcepts.Add(pConcept);
				FLAG = 1;
				break;
			}
		}
		...
		if (FLAG == 0){ 
			cerr << _T("CTDSetValue::buildBitValue: No match found for '")<< valueStr << _T("'")<<endl;
			return false;	
		}
	}
		 
	if (m_pRawConcepts.GetSize() == 0) {
		cerr << _T("CTDSetValue::buildBitValue: No set valuess.") << endl;
        return false;
    }
