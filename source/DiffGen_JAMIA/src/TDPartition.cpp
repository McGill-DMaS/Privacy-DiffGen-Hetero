...
bool CTDPartition::initGenRecords( CTDAttribs* pAttribs)
{
	...
	for (int classInd = 0; classInd < m_nClasses; ++classInd){
		...
		for (int attribID = 0; attribID < nAttribs; ++attribID){
			...
			else if (pAttrib->isSetValued())
				pNewValue = new CTDSetValue();
			else
		...
		for (int attribID = 0; attribID < nAttribs; ++attribID){
			...
			if (attribID == pAttribs->GetSize() - 1) {
				...
				if (pAttrib->isSetValued()){
					CTDHierarchyCut* pParentHierarchyCut = pParentPartition->getGenRecords()->GetAt(0)->getValue(splitIdx)->getCurrentHierarchyCut();
					ASSERT(pParentHierarchyCut);
					if(!pNewValue->setSplitCurHierarchyCut(pParentHierarchyCut, childInd))
						return false;
				}
				else{
			...
			{	// Other attributes
				// Initialize the current concept to the root concept. 
				if (pAttrib->isSetValued()){
					if(!pNewValue->setCurHierarchyCut(pParentPartition->getGenRecords()->GetAt(0)->getValue(attribID)->getCurrentHierarchyCut()))
						return false;
				}
				else {      
...
bool CTDPartition::deregisterPartition()
{
	...
	CTDHierarchyCut* pCurrentHierarchyCut = NULL;
    for (POSITION pos = m_partAttribs.GetHeadPosition(); pos != NULL; ++a) {
        pPartAttrib = m_partAttribs.GetNext(pos);
		// If it is a set-valued attribute, then deregister it in the hierarchyCut. 
		if (pPartAttrib->getActualAttrib()->isSetValued()){		
			 pCurrentHierarchyCut = pFirstRec->getValue(a)->getCurrentHierarchyCut();
			 if (!pCurrentHierarchyCut) {
				ASSERT(false);
				return false;
			 }
			 pCurrentHierarchyCut->deregisterPartition(pPartAttrib->m_relatedPos);
...
bool CTDPartition::testDeregisterPartition()
{
    ...
    for (POSITION pos = m_partAttribs.GetHeadPosition(); pos != NULL; ++a) {
		...
		if (pPartAttrib->getActualAttrib()->isSetValued()){		
			 ...
			 pCurrentHierarchyCut->testDeregisterPartition(pPartAttrib->m_relatedPos);
			 pPartAttrib->m_relatedPos = NULL;
		}
		else {
			pCurrentConcept = pFirstRec->getValue(a)->getCurrentConcept();
...
bool CTDPartition::registerPartition()
{
    ...
    for (POSITION pos = m_partAttribs.GetHeadPosition(); pos != NULL; ++a) {
		...
		// If it is a set-valued attribute, then register it in the hierarchyCut. 
		if (pPartAttrib->getActualAttrib()->isSetValued()){
			 ...
			 pPartAttrib->m_relatedPos = pCurrentHierarchyCut->registerPartition(this);
		}
		else {
			 pCurrentConcept = pFirstRec->getValue(a)->getCurrentConcept();
			 if (!pCurrentConcept) {
...
bool CTDPartition::testRegisterPartition()
{
    ...
    for (POSITION pos = m_partAttribs.GetHeadPosition(); pos != NULL; ++a) {
		...
		if (pPartAttrib->getActualAttrib()->isSetValued()){
			 ...
			 pPartAttrib->m_relatedPos = pCurrentHierarchyCut->testRegisterPartition(this);
		}
		else {
			 pCurrentConcept = pFirstRec->getValue(a)->getCurrentConcept();
			 if (!pCurrentConcept) {
...
bool CTDPartition::constructSupportMatrix(double epsilon)
{
    ...
    for (POSITION pos = m_partAttribs.GetHeadPosition(); pos != NULL; ++a) {
		...
		// we don't compute support matrix for set-valued attribute
		if (pPartAttrib->getActualAttrib()->isSetValued())
			continue;
...
bool CTDPartition::addNoise(double epsilon)
{
	...
	for (int j = 0; j < m_nClasses; ++j){
		...
		if (m_classNoisySums[j] <= (sqrt(2.0)/epsilon))
