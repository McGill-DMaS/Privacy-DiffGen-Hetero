...
CTDAttrib::CTDAttrib(LPCTSTR attribName)
    : m_attribName(attribName), m_pConceptRoot(NULL), m_numLeafNodes(0), m_attribIdx(-1), m_nOFlatConcepts(-1), m_bVirtualAttrib(false), m_bMaskTypeSup(false)
{
}
...
CTDSetAttrib::CTDSetAttrib(LPCTSTR attribName) 
    : CTDAttrib(attribName)
{
	m_pHierarchyCutRoot = NULL;	
}

CTDSetAttrib::~CTDSetAttrib() 
{
	delete m_pHierarchyCutRoot;
	m_pHierarchyCutRoot = NULL;
}

//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
bool CTDSetAttrib::initHierarchy(LPCTSTR conceptStr)
{
	
    ASSERT(!m_pConceptRoot);
    m_pConceptRoot = new CTDSetConcept(this);
    ...
    if (!flattenHierarchy())
	 ...
	 // initialize and register the hierarchycut for the set-valued attribute

	 m_pHierarchyCutRoot = new CTDHierarchyCut(this);
	 m_pHierarchyCutRoot->m_conceptsValues.Add(m_pConceptRoot);
	 m_pHierarchyCutRoot->registerHierarchyCut();
	 m_pHierarchyCutRoot->m_flattenIdx = getHierarchyCuts()->Add(m_pHierarchyCutRoot);

	#ifdef _DEBUG_PRT_INFO
		cout << _T("Reconstructed attribute: ") << m_attribName << endl;
		for (int c = 0; c < m_flattenConcepts.GetSize(); ++c) {
			cout << m_flattenConcepts.GetAt(c)->m_conceptValue << _T("(")<< m_flattenConcepts.GetAt(c)->m_minimum <<_T("-") << m_flattenConcepts.GetAt(c)->m_maximum << _T(")  ");
		}
    cout << endl;
	cout<< m_pConceptRoot->getNumChildConcepts()<<endl;
	cout<<m_flattenConcepts.GetSize()<<endl;
	#endif
